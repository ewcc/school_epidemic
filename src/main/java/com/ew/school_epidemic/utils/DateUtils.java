...
public class DateUtils {
     ...
     * @return
     */
    public static String getDateTimeLastTime(int time) {
        Date now = new Date();
        Date now_10 = new Date(now.getTime() - time); //10分钟前的时间600000
    ...
    public static int getDifferenceDateTime(String begin) {
        try {
            //1.获取当前时间
            Date dd = new Date();
            ...
            try {
                //获取两个时间的毫秒时间差
                diff = format.parse(end).getTime() - format.parse(begin).getTime();
            } catch (Exception e) {
                e.printStackTrace();
            }
            //计算相差的天数
            ...
            System.err.println(diff + "时间相差：" + day + "天" + hour + "小时" + min + "分钟" + sec + "秒。");

            int time = ((int) day * 24) + (int) hour;
